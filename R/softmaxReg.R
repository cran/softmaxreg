...
trainModel<-function(x, y, hidden, funName, maxit, rang, type, algorithm, rate, L2, penalty, threshold, batch){
	...
	for (k in 1:maxit) {
        ...
        for (j in 1:length(rand)) { # iteration over mini-batches
            ...
            loss = c(loss, curLoss)
            cat("Complete sample number: ", sampFin ," Current Loss Function Value ", curLoss," Best Loss Function Value ", minLoss, "\n")
